...
pentest goes from a UI click to a real, source-aware security scan.
...
strix/                    # upstream pentest engine — untouched except for
                           # one deliberate, documented exception (§8)
...
3. **The job queue is intentionally single-worker.** One `asyncio.Queue`
   plus one consumer task processes pentests strictly one at a time. This
   is not a scaling limitation that got missed — it's a chosen constraint
   that makes per-org LLM credential swapping via process env vars safe
   (§6.3). Parallelizing it later requires solving that isolation problem
   first (e.g. one subprocess per scan).
...
├── jobs.py               # async job queue + pentest execution (§6, §7)
...
└── routers/                   # one module per resource, one APIRouter each
    ...
    ├── pentests.py, issues.py      # scan lifecycle, findings
    ├── pr_reviews.py                # PR review trigger + settings + webhook receiver
    ├── knowledge.py, chat.py         # knowledge base, chat (partly mocked)
...
```python
_queue: asyncio.Queue[str] | None = None   # rebuilt per app lifespan
...
- **`Issue.repository_id` / `Issue.domain_id` / `Issue.pentest_id` /
  ...
  run or a PR review), never both sources at once.
...
/projects?membership=true`) instead of a fixed mock catalog.

## 7. The mock-vs-real provider pattern

Four things in this product need credentials only the operator/org can
provide. Each follows the same shape: a small interface, a `Mock*`
...
## 8. The one deliberate exception to "never touch `strix/`"
...
`parallel_tool_calls` was added to the body.

The fix — attaching `extra_body: {"allowed_openai_params":
["parallel_tool_calls"]}`, a client-side LiteLLM directive that needs no
server-side config change — required editing the engine, not `saas/`. This
was done explicitly, at the user's request, after confirming no `saas/` or
engine bug was at fault; the tradeoff (this file may need manual
reconciliation on a future `upstream/main` sync) was accepted knowingly
...
│   ├── Dashboard/
│   ├── Repositories/, Domains/       # add/list/remove targets
│   ├── Pentests/                       # trigger, list, live-polling detail view
│   ├── Issues/                           # list (with repo/pentest filters), detail
│   ├── PRReviews/                          # trigger, settings, connect-repo modal
│   ├── Knowledge/, Chat/
