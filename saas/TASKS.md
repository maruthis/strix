...
- [x] `P10-7` **Integrations page** (`pages/Integrations/IntegrationsList.tsx`) — previously a locked nav placeholder, now fully built and unlocked, matching the app.strix.ai screenshots: three sections (Code Providers: GitHub/GitLab/Bitbucket, Communication: Slack/Microsoft Teams, Issue Tracking: Jira/Linear), each row showing Connected status + account label or a Connect button; Microsoft Teams stays a static "Coming soon" row (matching the screenshot), rejected server-side too if POSTed directly. "Connect" opens `ConnectProviderModal.tsx`, which asks for the real account/workspace name plus an optional access token before calling the backend — not an instant fake connect. Backend: `Integration` model/table (`org_id`, `provider`, `account_label`, `credential_last4`, `connected_at`, unique per org+provider) and `app/routers/integrations.py` (`GET/POST /api/integrations/{provider}/connect`, `DELETE /api/integrations/{provider}`, all writes admin-gated). Like the GitHub App/Stripe integrations, there's no real OAuth handshake behind "Connect" — it's a real per-org DB row built from what the caller typed, but there's no live API call to the provider; if a credential is supplied only its last 4 characters are ever persisted (`credential_last4`, display-only — nothing calls out with it, so there's no reason to hold the full value). The demo org seed includes a connected GitHub integration (`account_label="maruthis"`) matching the screenshots. For GitHub and GitLab, "Configure" is a real external link (`configure_url` in the catalog: `github.com/settings/installations`, `gitlab.com/-/user_settings/applications`) opened in a new tab — GitHub also gets an in-app "Connect more" linking to the Repositories page, the actual place installed repos are managed. Other providers expose Connect/Disconnect only, since there's no per-provider settings surface (`configure_url`) defined for them yet — add one to the catalog entry to light it up for a new provider.
...
- [x] `PX-5` Automated test suites, not end-to-end browser tests specifically (still no headless-browser tool in this environment — see below). Backend: `saas/backend/tests/` — 165 pytest tests against a real per-test SQLite DB and a real FastAPI lifespan (job queue actually runs), **100% line coverage**, `fail_under=95` enforced in `pyproject.toml`. Frontend: `saas/frontend/src/**/*.test.tsx` — 198 Vitest + React Testing Library tests against every component/page with mocked `fetch`, **99.8% statement/line, 97.42% branch, 93.98% function coverage**, thresholds enforced in `vite.config.ts`. Writing these surfaced and fixed two real backend bugs: a cross-event-loop bug in the job queue's module-global `asyncio.Queue` (broke under multiple app lifespans in one process, e.g. tests), and pentests getting stuck in `"running"` forever on an unhandled scan exception instead of being marked `"failed"`. What's still not covered: true end-to-end browser tests (Playwright/Cypress) clicking through the real rendered UI against the real backend — see `saas/README.md`'s "Running locally" flow for manual verification steps instead.
